  ...
  export async function GET(req: Request) {
    ...
    try {
      for (const title of relatedJobTitles) {
        ...
        console.log(`✅ Scraped Indeed for: ${title}`);
  
        const indeedJobs = await page.evaluate(() => {
          return Array.from(document.querySelectorAll(".job_seen_beacon")).map(job => ({
            title: job.querySelector("h2.jobTitle span[aria-hidden='false']")?.textContent?.trim() || 
                   job.querySelector("h2.jobTitle span")?.getAttribute("aria-label") || 
                   "Unknown",
        ...
        console.log(`🔹 Found ${indeedJobs.length} Indeed jobs for ${title}`);
